...
const generateEmptyEntry = (attributes) => {
  ...
  for (const key of Object.keys(attributes)) {
    // Insert underscores before uppercase letters, lowercase, replace spaces with underscores, remove non-alphanumerics except underscore
    const newKey = key
      .replace(/([a-z0-9])([A-Z])/g, "$1_$2") // camelCase to snake_case
      .replace(/\s+/g, "_") // spaces to underscores
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, ""); // remove non-alphanumerics except underscore
    emptyEntry[newKey] = "";
  }
  return emptyEntry;
};
