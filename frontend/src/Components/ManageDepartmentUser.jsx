...
import DepartmentAdminUserInsights from "../Views/DepartmentAdminUserInsights";
...
const ManageDepartmentUser = ({ user, onBack, fetchAllUsers, department }) => {
  ...
  async function getAllUserConnections() {
    ...
    try {
      ...
      for (const connection of retrievedUserConnections) {
        ...
        if (
          userDetails.primary_department === department ||
          userDetails.secondary_department === department ||
      ...
      const filteredUserConnections = departmentUserConnections.filter(
        (connection) =>
      ...
      setPendingConnections(filteredUserConnections.filter((c) => c.status === "pending"));
      setConfirmedConnections(filteredUserConnections.filter((c) => c.status === "confirmed"));
    } catch (error) {
  ...
  const handleChangeRole = () => setIsChangeRoleModalOpen(true);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-6">
      ...
      {/* <div className="bg-white rounded-lg shadow px-12 py-8 flex flex-col gap-1">
        ...
        <div className="text-md text-zinc-500">({user.email})</div>
        <div className="text-md text-zinc-500 capitalize">{user.role}</div>
      </div> */}
      <div className="bg-white rounded-lg shadow px-12 py-8 flex flex-col gap-1">
...
export default ManageDepartmentUser;
