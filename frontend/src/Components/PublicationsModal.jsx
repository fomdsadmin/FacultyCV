...
const PublicationsModal = ({ user, section, onClose, setRetrievingData, fetchData }) => {
  ...
  return (
    <dialog className="modal-dialog" open>
      <button type="button" className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4" onClick={onClose} disabled={fetchingData && !initialRender}>✕</button>
      {initialRender ? (
        <div className='flex items-center justify-center w-full mt-5 mb-5'>
          <button type="button" className="btn btn-secondary" onClick={() => fetchPublicationsData()}>Fetch Publications</button>
        </div>
      ) : (
        fetchingData ? (
          <div className='flex items-center justify-center w-full mt-5 mb-5'>
