...
// Terminated Users Modal Component
export const TerminatedUsersModal = ({
  isOpen,
  onClose,
  terminatedUsers,
  searchTerm,
  ...
  if (userRole === "Admin" || userRole === "Admin-All") {
    // Show all departments
    departments = Array.from(new Set(terminatedUsers.map((user) => user.primary_department).filter(Boolean))).sort();
  } else if (userRole && userRole.startsWith("Admin-")) {
    // Show only the admin's department
    const adminDept = userRole.replace("Admin-", "");
    departments = terminatedUsers.some((user) => user.primary_department === adminDept) ? [adminDept] : [];
  } else {
    // Fallback to all departments for other roles
    departments = Array.from(new Set(terminatedUsers.map((user) => user.primary_department).filter(Boolean))).sort();
  }

  const filteredTerminatedUsers = terminatedUsers
    .filter((user) => {
      const firstName = user.first_name || "";
      const lastName = user.last_name || "";
      const email = user.email || "";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
          <div>
            <h3 className="text-xl font-semibold text-gray-800">
              Terminated Members ({filteredTerminatedUsers.length} of {terminatedUsers.length})
            </h3>
            <p className="text-sm text-gray-600 mt-1">
              Members who have been marked as terminated on Workday.
            </p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 overflow-hidden p-4">
          {filteredTerminatedUsers.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {searchTerm || departmentFilter
                    ? "No terminated members found matching your criteria."
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 h-full overflow-hidden flex flex-col">
              <div className="overflow-auto flex-1">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredTerminatedUsers.map((user, index) => (
                      <tr
                        key={user.user_id}
                        className={`transition-colors duration-150 hover:bg-gray-50 ${
                          index % 2 === 0 ? "bg-white" : "bg-gray-50/30"
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="text-sm font-medium text-gray-700">
                            {getPrimaryRank ? (
                              getPrimaryRank(user.user_id) || <span className="text-gray-400 italic">Not specified</span>
                            ) : (
