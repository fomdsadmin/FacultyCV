    ...
    try {
      ...
      if (response?.other_data) {
        ...
        const transformedData = educationList.map((education) => {
          ...
          const startDateYear =
            education["start_year"] && education["start_year"] !== "N/A" ? education["start_year"] : ""; // Set to empty string if "N/A"
          const endDateMonth =
      ...
      {initialRender ? (
        user.orcid_id ? (
          <div className="flex flex-col items-center justify-center w-full mt-5 mb-5">
            <div className="text-center">Education data will be fetched from ORCID using your ORCID ID.</div>
            <button type="button" className="btn btn-secondary mt-4 text-white" onClick={() => fetchEducationData()}>
              Fetch Education Data
            </button>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center w-full mt-5 mb-5">
            <span className="block text-m text-center mb-1 mt-6 mr-5 ml-5 text-zinc-600">
                Please enter your ORCID ID in the Profile section
                <br />
                to fetch education data.
            </span>
              <button type="button" className="btn btn-secondary mt-4 text-white"
        ...
        <div className="flex items-center justify-center w-full mt-5 mb-5">
          <div className="block text-lg font-bold mb-2 mt-6 text-zinc-600">Fetching education data...</div>
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center w-full mt-5 mb-5">
          {educationData.length > 0 ? (
            <div>
              ...
              <p className="mb-3" style={{ color: "red" }}>
                {" "}
                It is advised that you add the start and end dates to your education records.
              </p>
              <button
                type="button"
