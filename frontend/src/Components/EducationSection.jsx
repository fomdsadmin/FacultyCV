...
const EducationSection = ({ user, section, onBack = null }) => {
  ...
  const getUniqueDropdownValues = (attribute) => {
    ...
    fieldData.forEach((entry, index) => {
      if (!entry.data_details || !entry.data_details[actualKey]) {
        console.log(`Entry ${index}: Missing data_details or key ${actualKey}`);
        return;
      }
      ...
      if (filterValue && filterValue !== "all") {
        ...
        filtered = filtered.filter((entry) => {
          ...
          if (typeof entryValue === "string" && /\bother\b/i.test(entryValue) && /\(.*\)$/.test(entryValue)) {
            ...
            if (match && match[2] && match[2].trim() !== "") {
              return match[2].trim() === filterValue;
            }
          }
  ...
  const paginatedData = pageSize === 1000 ? filteredData : filteredData.slice(startIndex, startIndex + pageSize);
  ...
  return (
    <div>
      {/* Header Section - matches EmploymentSection layout */}
      ...
      {/* Description Section - matches EmploymentSection layout */}
      ...
      {/* Filters and Search - matches EmploymentSection layout */}
      {fieldData.length > 0 && (
        <div className="mb-2 bg-white px-4 py-3 rounded-lg shadow-md mt-2 ml-4 mr-4">
          ...
          <div className="flex flex-wrap gap-3 items-end mb-1">
                  ...
                  if (optionValue === "all") {
                    ...
                    return dataForCounting.filter((entry) => {
                      ...
                      if (
                        ...
                        const match = entryValue.match(/^(.*Other)\s*\((.*)\)$/i);
                        if (match && match[2] && match[2].trim() !== "") {
                          return match[2].trim() === optionValue;
                        }
                      }
            ...
            {getDropdownAttributes().length > 4 && (
              <button
          ...
          {showMoreFilters && getDropdownAttributes().length > 4 && (
