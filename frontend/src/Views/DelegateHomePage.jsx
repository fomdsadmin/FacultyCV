...
const DelegateHomePage = ({ assistantUserInfo, userInfo, setUserInfo, getUser, getCognitoUser }) => {
  ...
  const sortUniversityInfo = () => {
    getAllUniversityInfo().then((result) => {
      ...
      const faculties = result
        ...
        .map((item) => item.value)
        .sort();
      const campuses = result
        .filter((item) => item.type === "Campus")
        .map((item) => item.value)
        .sort();
      const ranks = result
  ...
  return (
    <div>
      ...
      <AssistantPageContainer>
        <DelegateMenu userInfo={userInfo} assistantUserInfo={assistantUserInfo}></DelegateMenu>
        <main className="px-[2vw] md:px-[3vw] lg:px-[5vw] overflow-auto custom-scrollbar w-full mb-4 relative">
          <div className="flex items-center justify-between mt-4 mb-4">
            <h1 className="text-4xl ml-4 font-bold text-zinc-600">Delegate Profile</h1>
            <button
              type="button"
