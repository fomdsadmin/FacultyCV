...
const GenerateCV = ({ getCognitoUser, toggleViewMode }) => {
    ...
    useEffect(() => {
        ...
        if (selectedFaculty && selectedFaculty !== "") {
            filteredUsers = filteredUsers.filter(user =>
                user.primary_faculty === selectedFaculty ||
                user.secondary_faculty === selectedFaculty
            );
        }
    ...
    // Set all users to departmentUsers (no filtering)
    useEffect(() => {
    ...
    const SelectedUsersSummary = () => (
        selectedUsers.length > 0 && canSelectMultiple && (
            <div className="mb-6">
                        ...
                        <div className="w-1/3 space-y-6">
                            ...
                            <div className={`${selectedUsers.length === 0 || !selectedTemplate ? 'opacity-50 pointer-events-none' : ''}`}>
                                <CVGenerationComponent
                                    ...
                                    pdfGenerationCompleteMessage={`PDF for ${canSelectMultiple ? 'bulk ' : ''}CV "${selectedTemplate?.title}" finished generating!`}
                                    docxGenerationCompleteMessage={`DOCX for ${canSelectMultiple ? 'bulk ' : ''}CV "${selectedTemplate?.title}" finished generating!`}
                                />
                            </div>
                        </div>
