...
const TablesManager = ({ sqlSettings = {}, setSqlSettings }) => {
    ...
    const currentDataSources = sqlSettings?.dataSources || [];
    ...
    const handleAddTable = () => {
        ...
        if (currentDataSources.some((table) => table.tableName === tableName)) {
            setError("Table name already exists");
            return;
        }

        // Add new table to dataSources
    ...
    const handleRemoveTable = (tableName) => {
        const updatedDataSources = currentDataSources.filter(
    ...
    return (
        <div className="mb-4 p-3 bg-white rounded border border-gray-300">
            <div className="mb-3">
                <h4 className="text-xs font-semibold text-gray-800 mb-2">Data Sources</h4>
                <p className="text-gray-600 text-xs mb-3">
                    Add data sources to your query. You can join multiple tables using the table names you specify.
                </p>
            </div>
            ...
            {currentDataSources.length > 0 ? (
                <div className="space-y-2">
                    {currentDataSources.map((table, index) => (
                        <div
