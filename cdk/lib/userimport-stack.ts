...
export class UserImportStack extends Stack {
  ...
  public readonly dataMigrationAPTS3Bucket: s3.Bucket;
  ...
  public getDataMigrationAPTBucketName = () => this.dataMigrationAPTS3Bucket.bucketName;
  public getDataMigrationAPTBucketArn = () => this.dataMigrationAPTS3Bucket.bucketArn;
  ...
  constructor(
    ...
    const createDataMigrationObgynFolders = new triggers.TriggerFunction(
      this,
      "facultyCV-createDataMigrationObgynFolders",
      {
        runtime: lambda.Runtime.PYTHON_3_9,
    ...
    // Create folder structure for data migration of APT data
    const createDataMigrationAPTFolders = new triggers.TriggerFunction(
      this,
      "facultyCV-createDataMigrationAPTFolders",
      {
        runtime: lambda.Runtime.PYTHON_3_9,
        functionName: `${resourcePrefix}-createDataMigrationAPTFolders`,
        handler: "createDataMigrationAPTFolders.lambda_handler", // reuse same handler
        code: lambda.Code.fromAsset("lambda/create-data-migration-APT-folders"),
        timeout: cdk.Duration.minutes(1),
        memorySize: 512,
        vpc: vpcStack.vpc,
        environment: {
          BUCKET_NAME: this.dataMigrationAPTS3Bucket.bucketName,
        },
      }
    );
    createDataMigrationAPTFolders.addToRolePolicy(
      new iam.PolicyStatement({
        effect: Effect.ALLOW,
        actions: ["s3:PutObject", "s3:GetObject", "s3:DeleteObject"],
        resources: [`arn:aws:s3:::${this.dataMigrationAPTS3Bucket.bucketName}/*`],
      })
    );
    createDataMigrationAPTFolders.executeAfter(this.dataMigrationAPTS3Bucket);
    ...
    processUserImport.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `arn:aws:secretsmanager:${Stack.of(this).region}:${Stack.of(this).account}:secret:facultyCV/credentials/*`,
        ],
      })
    );
    processManualUpload.addToRolePolicy(
    ...
    // List of APT subfolders and their corresponding Lambda code paths
    const aptSubfolders = [
      "approach_teachings",
      "categories",
      "cit",
      "clinically_integrated_teachings",
      "continuing_education_activities",
      "continuing_education_trainings",
      "curriculum_development_innovations",
      "educational_conferences",
      "educational_leaderships",
      "efforts_improve_teachings",
      "employment_records",
      "leave_of_absences",
      "meritform",
      "merits",
      "other_teaching_learning_activities",
      "other_teachings",
      "other_types_scholarly_contributions",
      "research_equivalent_grants_contracts",
      "special_professional_qualifications",
      "supervising_advising_students",
      "teaching_awards",
      "templates",
      "trainee_awards",
      "ubc_courses",
    ...
    // For each subfolder, create a Lambda and add S3 event notification
    aptSubfolders.forEach((subfolder) => {
      const lambdaFn = new lambda.Function(this, `facultyCV-APT-${subfolder}-import`, {
        runtime: lambda.Runtime.PYTHON_3_9,
        functionName: `${resourcePrefix}-APT-${subfolder}-import`,
        handler: "lambda_handler.lambda_handler",
        code: lambda.Code.fromAsset(`APT-CV-import-scripts/${subfolder}`),
        timeout: cdk.Duration.minutes(5),
        memorySize: 512,
        environment: {
          DB_PROXY_ENDPOINT: databaseStack.rdsProxyEndpoint,
      ...
      lambdaFn.addToRolePolicy(
        new iam.PolicyStatement({
          ...
          resources: [
            `arn:aws:s3:::${this.dataMigrationAPTS3Bucket.bucketName}`,
            `arn:aws:s3:::${this.dataMigrationAPTS3Bucket.bucketName}/*`,
          ],
        })
      );
      ...
      // S3 invoke permissions
      lambdaFn.addPermission(`s3-invoke-APT-${subfolder}`, {
        principal: new iam.ServicePrincipal("s3.amazonaws.com"),
        action: "lambda:InvokeFunction",
        sourceAccount: Stack.of(this).account,
        sourceArn: this.dataMigrationAPTS3Bucket.bucketArn,
      });
      ...
      // S3 event notification for this subfolder
      this.dataMigrationAPTS3Bucket.addEventNotification(
        s3.EventType.OBJECT_CREATED_PUT,
        new s3n.LambdaDestination(lambdaFn),
        {
          prefix: `APT/${subfolder}/`,
        }
      );
    });
  }
