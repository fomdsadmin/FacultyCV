...
def get_dynamodb_item(cognito_user_id, user_id, template_id=None):
    try:
        response = None
        if template_id is not None:
            response = dynamodb.get_item(Key={"logEntryId": {"S": f"{cognito_user_id}/{user_id}/{template_id}"}}, TableName=os.environ['TABLE_NAME'])
        else:
            response = dynamodb.get_item(Key={"logEntryId": {"S": f"{cognito_user_id}/{user_id}"}}, TableName=os.environ['TABLE_NAME'])
...
def lambda_handler(event, context):
    ...
    dynamodb_item_user = get_dynamodb_item(cognito_user_id, user_id)
    ...
    dynamodb_item_template = get_dynamodb_item(cognito_user_id, user_id, template_id)
