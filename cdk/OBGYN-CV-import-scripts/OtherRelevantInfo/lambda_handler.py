...
def cleanData(df):
    ...
    # Handle highlight column properly - convert string/numeric values to boolean
    if "Highlight" in df.columns:
        df["highlight"] = df["Highlight"].fillna(False)
        # Convert string representations to boolean
        df["highlight"] = df["highlight"].astype(str).str.lower().isin(['true'])
    else:
        df["highlight"] = False
    ...
    df = df[["user_id", "details", "highlight_-_notes", "highlight", "dates"]]
