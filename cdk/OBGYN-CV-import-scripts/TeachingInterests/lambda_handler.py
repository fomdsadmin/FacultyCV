...
SECTION_TITLE = "8a. Areas of Special Interest and Accomplishments"
...
def cleanData(df):
    ...
    # Handle Highlight column - convert TRUE/FALSE to boolean
    if "Highlight" in df.columns:
        df["highlight"] = df["Highlight"].fillna('').astype(str).str.upper().str.strip() == 'TRUE'
    else:
        df["highlight"] = False

    # If Type is "Teaching Interests", set type to "Areas of Special Interests and Accomplishments"
    mask_teaching = df["Type"].fillna('').str.strip() == "Teaching Interests"
    df.loc[mask_teaching, "type"] = "Areas of Special Interests and Accomplishments"
    
    # If Type is "Other Teaching Interests", set type to "Other ({TypeOther})"
    mask_other = df["Type"].fillna('').str.strip() == "Other Teaching Interests:"
