...
SECTION_TITLE_A = "5c. Continuing Education or Training"
...
def cleanData(df):
    ...
    for col in ["user_id", "university_or_institution", "rank_or_title" ]:
        if col in a.columns:
    ...
    a["university/organization"] = safe_string_clean(a["university_or_institution"]) if "university_or_institution" in a.columns else ""
    ...
    available_cols_a = []
    for col in ["user_id", "university/organization", "rank_or_title", "dates"]:
        if col in a.columns:
    ...
    a = a[["user_id", "university/organization", "rank_or_title", "dates"]]
