...
SECTION_TITLE_A = "8e. Continuing Education Activities"
...
def cleanData(df):
    ...
    for col in ["user_id", "description", "role", "role_other"]:
        if col in a.columns:
    ...
    a["title/rank"] = safe_string_clean(a["description"]) if "description" in a.columns else ""
    
    # Map category_id values to type column with custom mapping
    def map_role_to_type(row):
        role = str(row.get('role', '')).strip()
        role_other = str(row.get('role_other', '')).strip()
        role_mapping = {
            'CME Attendee': 'CME Attendee',
            'CME Presenter or Facilitator': 'CME Teacher/Presenter/Facilitator',
        }
        
        if role == '':
            return f'Other ({role_other})' if role_other else 'Other'
        
        # Get the mapped value or use role as fallback
        if role in role_mapping:
            mapped_value = role_mapping[role]
            return mapped_value
        else:
            return role
        
    # Apply the mapping logic to create type and title columns
    a["role"] = safe_string_clean(a["role"]) if "role" in a.columns else ""
    a["role_other"] = safe_string_clean(a["role_other"]) if "role_other" in a.columns else ""
    a["type"] = a.apply(map_role_to_type, axis=1)
    ...
    a = a[["user_id", "title/rank", "type", "dates"]]
