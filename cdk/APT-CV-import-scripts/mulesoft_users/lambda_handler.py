...
def processUserData(user_data_list, connection, cursor):
    ...
    # Create name-based mapping for matching
    for json_user in user_data_list:
        try:
            ...
            # Find matching user in database by name
            matched_user = None
            for db_user in apt_users:
                ...
                if given_name == db_given and family_name == db_family:
                    matched_user = db_user
                    break
            
            if not matched_user:
                errors.append(f"No database match found for {json_user.get('givenName', '')} {json_user.get('familyName', '')}")
                continue
            
            matched_users += 1
            user_id = matched_user['user_id']
            
            # Update user information (cwl_username and employee_id)
            update_result = updateUserInfo(json_user, user_id, cursor)
...
def updateUserInfo(user_data, user_id, cursor):
    """
    Update user information in the users table - cwl_username, employee_id, active, terminated, and email
    """
    try:
