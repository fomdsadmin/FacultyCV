...
def cleanData(df):
    ...
    def map_category_to_type(row):
        ...
        category_mapping = {
            '3001': 'Undergraduate Students (BA/BSc/Co-op/Directed Studies/MD/Summer Student)',
            '3002': 'Undergraduate Students (BA/BSc/Co-op/Directed Studies/MD/Summer Student)',
            '3003': 'Graduate Students (MA/MPH/MSc/PhD/Postdoctoral Study)',
            '3004': 'Postgraduate Students (Fellowship/Residency)',
            '3006': 'Graduate Studies Examination Committees',
            '3008': 'Research Personnel Supervision',
        }
        ...
        if category_id in category_mapping:
            ...
            return None  # Return None for unmapped categories (including 3005)
        
    # Apply the mapping logic to create type column
    ...
    # Process dataframe B (entries with category_id 3005)
    b["user_id"] = b["user_id"].apply(process_user_id)
    b["user_id"] = b["user_id"].astype('Int64')
    b["category_id"] = safe_string_clean(b["category_id"]) if "category_id" in b.columns else ""
    
    # Filter dataframe B to only include entries where category_id is 3005
    b = b[b["category_id"] == "3005"]
