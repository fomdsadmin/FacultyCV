...
SECTION_TITLE_A = "6[a-d]. Employment Record"
...
def cleanData(df):
    ...
    for col in ["user_id", "university_company_organization", "rank_or_title", "event_category" ]:
        if col in a.columns:
    ...
    type_mapping = {
        ...
        'Present': 'c. Present', 
        'Affiliations': 'c. Present',
    }
    
    # Apply the mapping, defaulting to the original value if not found in mapping
    if "event_category" in a.columns:
        a["type"] = a["event_category"].fillna('').astype(str).map(type_mapping).fillna(a["event_category"].fillna('').astype(str))
    else:
        a["type"] = ""
    ...
    a = a[["user_id", "university/organization", "rank_or_title", "type", "dates"]]
