...
def cleanData(df):
    ...
    for col in ["user_id", "institution_name", "degree", "subject_area", "title_of_dissertation", "supervisor" ]:
        if col in a.columns:
    ...
    a["university/organization"] = safe_string_clean(a["institution_name"]) if "institution_name" in a.columns else ""
    a["degree"] = safe_string_clean(a["degree"]) if "degree" in a.columns else ""
    a["subject_area"] = safe_string_clean(a["subject_area"]) if "subject_area" in a.columns else ""
    a["supervisor"] = safe_string_clean(a["supervisor"]) if "supervisor" in a.columns else ""
    ...
    # Keep only the cleaned columns for dataframe A (Post-Secondary Education)
    available_cols_a = []
    for col in ["user_id", "university/organization", "degree", "subject_area", "supervisor", "dates"]:
        if col in a.columns:
            available_cols_a.append(col)
        else:
            a[col] = ""  # Add missing columns as empty strings
            available_cols_a.append(col)
    
    a = a[["user_id", "university/organization", "degree", "subject_area", "supervisor", "dates"]]
    ...
    for col in a.columns:
        a[col] = a[col].astype(str).replace(['nan', 'None', 'null', 'NULL'], '')
    
    # Process dataframe B (Dissertations)
    b["user_id"] = b["user_id"].apply(process_user_id)
    b["user_id"] = b["user_id"].astype('Int64')
    b["supervisor"] = safe_string_clean(b["supervisor"]) if "supervisor" in b.columns else ""
    b["title_of_dissertation"] = safe_string_clean(b["title_of_dissertation"]) if "title_of_dissertation" in b.columns else ""
    ...
    if 'end_year' in b.columns and 'end_month' in b.columns:
        b['end_date'] = b.apply(lambda row: format_date(row['end_year'], row['end_month']), axis=1)
    elif 'end_date' not in b.columns:
        b['end_date'] = ""

    # Ensure start_date and end_date are strings for dataframe B
    b['start_date'] = b['start_date'].astype(str).fillna('').str.strip()
    b['end_date'] = b['end_date'].astype(str).fillna('').str.strip()
    
    b["dates"] = b.apply(combine_dates, axis=1)
    
    # Filter dataframe B to only include entries where title_of_dissertation is not empty
    # First ensure title_of_dissertation is properly cleaned
    b["title_of_dissertation"] = b["title_of_dissertation"].fillna('').astype(str).replace(['nan', 'None', 'null', 'NULL'], '').str.strip()
    b = b[b["title_of_dissertation"] != ""]

    # Keep only the cleaned columns for dataframe B (Dissertations)
    available_cols_b = []
    for col in ["user_id", "supervisor", "subject_area", "title_of_dissertation", "dates"]:
        if col in b.columns:
            available_cols_b.append(col)
        else:
            b[col] = ""  # Add missing columns as empty strings
            available_cols_b.append(col)
    
    b = b[["user_id", "supervisor", "subject_area", "title_of_dissertation", "dates"]]
    ...
    for col in b.columns:
        b[col] = b[col].astype(str).replace(['nan', 'None', 'null', 'NULL'], '')
