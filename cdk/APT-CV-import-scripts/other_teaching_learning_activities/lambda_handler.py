...
def cleanData(df):
    ...
    for col in ["user_id", "category_other", "category", "description" ]:
        if col in a.columns:
    ...
    a["category_other"] = safe_string_clean(a["category_other"]) if "category_other" in a.columns else ""
    ...
    def map_category_to_type(row):
        """Map category_id to type with custom logic - all entries use 'i. Other' format"""
        ...
        category_mapping = {
            '4021': 'PBL Case Writer',
            '4022': 'Formal Student Mentoring', 
            '4023': 'Teaching Committee Membership',
            '4024': 'OSCE Examinations',
        }
        
        # Special handling for category_id 4025 - just return "i. Other"
        if category_id == '4025':
            return "i. Other"
    ...
    def map_category_to_title(row):
        """Map category_id to title - only for 4025, others get blank"""
        category_id = str(row.get('category_id', '')).strip()
        category_other = str(row.get('category_other', '')).strip()
        
        # Only for category_id 4025, put category_other in title
        if category_id == '4025':
    ...
    a["title"] = a.apply(map_category_to_title, axis=1)
    ...
    a = a[["user_id", "type", "title", "details", "dates"]]
